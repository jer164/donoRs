...
reticulate::virtualenv_install("python_environment", packages = c("pandas", "lxml", "bs4", "requests"))
...
donor_cleaner <- function(input_data_path, state_fin) {
  ...
  good_names <- c(
    "donation_date", "donation_amount", "full_address", "addr1", "addr2",
    "city", "state", "zip", "full_name", "first_name", "middle_name",
    "last_name", "phone1", "phone2", "email1", "email2"
  )
  ...
  ### DC also has a formatting issue and needs a skip
    ...
    temp_data <- read_csv(input_data_path,
      skip = 1, locale = readr::locale(encoding = "UTF-16LE")
    ) %>% as_tibble()
  }
  ...
  # }
    ...
    temp_data <- read_xlsx(input_data_path)
  }
  ...
  if (state_fin == "IL") {
    ...
    temp_data <- temp_data %>%
      mutate(last_name = str_remove(last_name, "Mr.|Ms.|Mrs.|Dr.")) %>%
    ...
    temp_data <- temp_data %>%
      mutate(first_name = word(full_name, 2, sep = "\\s{2}")) %>%
      mutate(last_name = word(full_name, 1, sep = "\\s{2}")) %>%
      mutate(full_name = "") %>%
      mutate(addr1 = word(full_address, 1, sep = "\\s{2}")) %>%
      mutate(addr2 = ifelse(str_count(full_address, "\\s{2}") == 3, word(full_address, 2, sep = "\\s{2}"),
        ""
      )) %>%
      mutate(city = ifelse(str_count(full_address, "\\s{2}") == 2, word(full_address, 2, sep = "\\s{2}"),
        word(full_address, 3, sep = "\\s{2}")
      )) %>%
      mutate(zip = str_extract(full_address, "(\\d{5})\\b")) %>%
      mutate(state = str_extract(full_address, "([A-Z]{2})\\b")) %>%
      mutate(full_address = "") %>%
      mutate(donation_amount = gsub("..$", "", donation_amount)) %>%
    ...
    temp_data <- temp_data %>%
      ...
      mutate(first_name = str_split_i(first_name, " ", 1)) %>%
    ...
    temp_data <- temp_data %>% 
      filter(c_code == 'IND') %>% 
      mutate(first_name = word(name, 2, sep = ",")) %>% 
      mutate(last_name = word(name, 1, sep = ",")) %>% 
      mutate(zip = str_extract(zip, "(\\d{5})"))
  ...
  } else if (state_fin == "SLC") {
    
    temp_data <- temp_data %>% clean_names()
    
    
    temp_data <- temp_data %>% 
      mutate(first_name = word(contributor, 2, sep = ",")) %>% 
      mutate(last_name = word(contributor, 1, sep = ",")) %>% 
      mutate(full_address = contributor_address) %>% 
      mutate(donation_amount = amount) %>% 
      mutate(donation_date = date_received)
    ...
    temp_data <- temp_data %>% 
      mutate(first_name = word(full_name, 2, sep = "  ")) %>% 
      mutate(last_name = word(full_name, 1, sep = " ")) %>% 
      mutate(zip = str_extract(zip, pattern = "\\d{5}")) %>% 
      mutate(across(where(is.character), ~ replace_na(., ''))) %>% 
      select(-full_name) %>% 
      filter(str_detect(addr1, "#") == F) %>% 
      filter(str_ends(addr1, "\\d") == F)
    ...
    temp_data <- temp_data %>%
      ...
      mutate(zip = str_extract(full_address, "(\\s\\d{5}\\b)")) %>%
      mutate(city = ifelse(str_count(full_address, ",") == 2, 
             word(full_address, 2, sep = ","), 
  ...
  temp_data <- temp_data %>%
    ...
    mutate(full_address = str_to_lower(full_address)) %>%
    mutate(across(c(
      addr1,
      addr2,
      city,
    ...
    distinct(first_name, last_name, full_name, addr1, addr2, .keep_all = T)
